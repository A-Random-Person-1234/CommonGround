...
- The current data store is a local JSON file called `.room-store.json`.
- Sessions are still in memory, so users may need to reconnect after a server restart.
- For a real public product, replace `.room-store.json` with a database and encrypted token storage.
