...
const outlookEventSyncInput = document.querySelector("#eventOutlookSyncInput");
const eventOutlookSyncStatus = document.querySelector("#eventOutlookSyncStatus");
...
const choiceConnectOutlookButton = document.querySelector("#choiceConnectOutlookButton");
...
const connectOutlookButton = document.querySelector("#connectOutlookButton");
const settingsReconnectButton = document.querySelector("#settingsReconnectButton");
const settingsReconnectOutlookButton = document.querySelector("#settingsReconnectOutlookButton");
...
const outlookEventSyncToggle = document.querySelector("#outlookEventSyncToggle");
const outlookEventSyncStatus = document.querySelector("#outlookEventSyncStatus");
...
function parseInputTime(value) {
  ...
  const hoursValue = Number(hourPart);
  const minutesValue = Number(minutePart);
...
function normalizeCustomRoomCodeInput(value) {
  return normalizeRoomCodeInput(value).replace(/[^A-Z]/g, "");
}
...
function outlookWriteReady() {
  return Boolean(sessionInfo?.user?.outlookWriteReady);
}
...
function outlookEventSyncEnabled() {
  return Boolean(outlookWriteReady() && sessionInfo?.user?.outlookEventSync?.enabled);
}
...
function renderRoomSwitcher() {
  ...
  const selectedCode = currentRoom?.code || activeRoomCode;
  for (const room of rooms) {
    ...
    mark.textContent = roomInitials(room);
  ...
  addButton.title = "Create another room";
  addButton.setAttribute("aria-label", "Create another room");
  addButton.innerHTML = `
    <span class="room-switch-mark" aria-hidden="true">+</span>
    <span class="room-switch-label">New room</span>
    <span class="room-switch-meta">Create</span>
  `;
  addButton.addEventListener("click", openCreateRoomModal);
...
function roomInviteMessage() {
  const roomLabel = `${currentRoom?.name || "Free Time room"}`.trim();
  const lockLine = currentRoom?.accessLocked
    ? "This room is locked, so open the link and send a join request for host approval."
    : "Open the link and join with the room code below.";
  return [
    `Join me on Free Time for ${roomLabel}.`,
...
async function copyInviteLink() {
  ...
  try {
    await navigator.clipboard.writeText(roomInviteMessage());
...
function renderInviteePicker(selectedIds = defaultInviteeIds()) {
  ...
  for (const participant of invitees) {
    ...
    label.innerHTML = `
      <input type="checkbox" value="${participant.id}" ${selected.has(participant.id) || isSelf ? "checked" : ""} ${isSelf ? "disabled" : ""} />
      <span class="invitee-color-dot"></span>
      <strong>${participant.displayName}${isSelf ? " (You)" : ""}</strong>
    `;
...
function renderJoinRequests() {
  ...
  for (const request of requests) {
    ...
    const sourceLabel = request.source === "microsoft" ? "Microsoft" : "Google";
    row.innerHTML = `
...
function renderCalendarEventSyncControls() {
  if (
    !googleEventSyncToggle ||
    !googleEventSyncStatus ||
    !outlookEventSyncToggle ||
    !outlookEventSyncStatus
  ) return;

  const connected = currentUserConnected() && currentParticipantConnected();
  const writeReady = calendarWriteReady();
  const outlookReady = outlookWriteReady();
  const enabled = calendarEventSyncEnabled();
  const outlookEnabled = outlookEventSyncEnabled();

  googleEventSyncToggle.checked = enabled;
  googleEventSyncToggle.disabled = !connected || !writeReady;
  outlookEventSyncToggle.checked = outlookEnabled;
  outlookEventSyncToggle.disabled = !connected || !outlookReady;

  if (!connected) {
    googleEventSyncStatus.textContent = "Connect a calendar to add event invites back to your calendar.";
    outlookEventSyncStatus.textContent = "Connect a calendar to add event invites back to your Outlook calendar.";
  } else if (!writeReady) {
    googleEventSyncStatus.textContent = "Reconnect Google Calendar once to allow Free Time to create event copies.";
  } else if (enabled) {
    googleEventSyncStatus.textContent = "Events you create or are invited to will be added to your Google Calendar.";
  } else {
    googleEventSyncStatus.textContent = "Off. Your Free Time events will stay inside this room only.";
  }

  if (!connected) {
    outlookEventSyncStatus.textContent = "Connect Outlook Calendar to add event invites back there too.";
  } else if (!outlookReady) {
    outlookEventSyncStatus.textContent = "Reconnect Outlook once to allow Free Time to create event copies.";
  } else if (outlookEnabled) {
    outlookEventSyncStatus.textContent = "Events you create or are invited to will be added to your Outlook calendar.";
  } else {
    outlookEventSyncStatus.textContent = "Off. Your Free Time events will stay inside this room only.";
  }
}
...
function renderRoomMeta() {
  ...
  if (currentParticipantNeedsReconnect()) {
    ...
    if (connectOutlookButton) connectOutlookButton.textContent = "Reconnect calendar";
    if (settingsReconnectOutlookButton) settingsReconnectOutlookButton.classList.remove("hidden");
    connectWidgetText.textContent = currentParticipant?.lastSyncError
      ? `Your calendar needs reconnecting. Last sync issue: ${currentParticipant.lastSyncError}`
      : "Your calendar needs reconnecting so Free Time can keep your live availability and profile name in sync.";
    ...
    if (connectOutlookButton) connectOutlookButton.textContent = "Calendar connected";
    if (settingsReconnectOutlookButton) settingsReconnectOutlookButton.classList.add("hidden");
    const syncStamp = formatSyncStamp(currentParticipant?.lastSyncedAt || sessionInfo?.user?.sync?.lastSuccessAt);
    const calendarName = currentParticipant?.lastCalendarName || sessionInfo?.user?.sync?.calendarName || "your primary calendar";
    connectWidgetText.textContent = syncStamp
      ? `Your calendar is connected. Free Time last synced ${calendarName} at ${syncStamp} and is using your profile as the default identity.`
      : "Your calendar is connected. Free Time is pulling your live availability and using your profile as the default identity.";
  } else {
    connectGoogleButton.textContent = "Connect calendar";
    settingsReconnectButton.classList.add("hidden");
    if (connectOutlookButton) connectOutlookButton.textContent = "Connect calendar";
    if (settingsReconnectOutlookButton) settingsReconnectOutlookButton.classList.add("hidden");
    connectWidgetText.textContent = "Connect a calendar to pull your real busy times automatically and fill in your name from your account profile.";
  }
...
function busySegmentsForDate(date) {
  ...
  for (const block of blocks) {
    if (!activeGroup || block.startHour >= activeGroup.endHour) {
      if (activeGroup) {
        segments.push({
          id: `${dateKey(date)}-${segments.length}-${activeGroup.startHour}-${activeGroup.endHour}`,
          date,
          startHour: activeGroup.startHour,
          endHour: activeGroup.endHour,
          participants: mergeParticipantEntries(activeGroup.blocks)
        });
      }
    ...
    activeGroup.endHour = Math.max(activeGroup.endHour, block.endHour);
  }
...
function invitedEventOverlap(participantId, date, startHour, endHour) {
  ...
  rangeStart.setHours(Math.floor(startHour), Math.round((startHour % 1) * 60), 0, 0);
  const rangeEnd = new Date(date);
  rangeEnd.setHours(0, 0, 0, 0);
  rangeEnd.setHours(Math.floor(endHour), Math.round((endHour % 1) * 60), 0, 0);
...
function createSingleBusyCard(segment, dayIndex) {
  ...
  const timeRange = formatEventRange(segment.startHour, segment.endHour);
...
function createBusyStack(segment, dayIndex) {
  ...
  stack.className = `busy-stack ${hasLaneOverlap ? "busy-overlap-lane" : ""}`.trim();
  ...
  summary.innerHTML = isFifteen
    ? `
      <div class="busy-stack-summary-body busy-stack-summary-body--single">
        <strong class="busy-stack-summary-inline">${countLabel} · ${formatTime(segment.startHour)} - ${formatTime(segment.endHour)}</strong>
      </div>
    ...
    : `
      <div class="busy-stack-summary-body">
        ...
        <span class="busy-stack-names">${namesLabel}${extraLabel}</span>
      </div>
      <div class="busy-stack-tabs"></div>
    `;
  ...
  for (const [index, participant] of segment.participants.entries()) {
    ...
    const participantInvitedOverlap = invitedEventOverlap(participant.participantId, segment.date, participantStartHour, participantEndHour);
    ...
    card.innerHTML = `
      ...
      <div class="busy-stack-item-copy">
        ...
        <span class="busy-visibility">${visibilityLabel}</span>
        <span class="busy-time">${participantTimeLabel}</span>
      </div>
    `;
...
function createEventBlock(item, dayIndex) {
  ...
  const compactLine = [ownerLabel, titleText].filter(Boolean).join(" · ");
  const fifteenLine = compactLine;
  ...
  if (durationClass === "event-15") {
    appendLine("event-line-compact", fifteenLine || compactLine);
  } else if (durationClass === "event-30") {
...
function dragTargetIsBlocked(target) {
  return Boolean(target.closest(
    ".event-card, .busy-card, .busy-stack, .busy-stack-popover, .busy-stack-item, .color-picker-menu, .host-popover, .detail-panel, .modal-card, .vote-button, .chip-action, .icon-button, .add-event-button, input, textarea, select, summary, label"
  ));
}
...
function openDraggedEventComposer(anchorRect) {
  ...
  if (!selection) return;
  if (!dragCreateState.hasMoved || selectionDurationHours(selection) < 0.26) {
...
async function refreshMyRooms() {
  try {
    const data = await fetchJson("/api/my-rooms");
    myRooms = Array.isArray(data.rooms) ? data.rooms : [];
    activeRoomCode = data.activeRoomCode || currentRoom?.code || null;
  } catch {
    myRooms = currentRoom ? [currentRoomSwitcherSummary()].filter(Boolean) : [];
    activeRoomCode = currentRoom?.code || null;
  }
  renderRoomSwitcher();
}
...
function shouldIgnoreUndoShortcut(target) {
  return shouldIgnoreViewShortcut(target) ||
    Boolean(eventModal?.open || busyModal?.open || proposeModal?.open || createRoomModal?.open);
}
...
async function createRoom(event) {
  ...
  try {
    ...
    const data = await fetchJson("/api/rooms", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: roomNameValue })
    });
...
async function saveRoomCode() {
  ...
  if (!code) {
    calendarStatus.textContent = "Enter a letters-only room code.";
    return;
  }
...
function updateEventGoogleSyncControl() {
  if (!eventGoogleSyncInput || !eventGoogleSyncStatus || !outlookEventSyncInput || !eventOutlookSyncStatus) return;
  const connected = currentUserConnected() && currentParticipantConnected();
  const googleWriteReady = calendarWriteReady();
  const outlookWriteReadyFlag = outlookWriteReady();
  eventGoogleSyncInput.disabled = !connected || !googleWriteReady;
  outlookEventSyncInput.disabled = !connected || !outlookWriteReadyFlag;

  if (!connected) {
    eventGoogleSyncStatus.textContent = "Connect Google Calendar first.";
    eventOutlookSyncStatus.textContent = "Connect Outlook Calendar first.";
  } else if (!googleWriteReady) {
    eventGoogleSyncStatus.textContent = "Reconnect in settings to enable Google event sync.";
  } else if (eventGoogleSyncInput.checked) {
    eventGoogleSyncStatus.textContent = "Adds this event to your Google Calendar.";
  } else {
    eventGoogleSyncStatus.textContent = "Keeps this event in Free Time only.";
  }

  if (!connected) {
    eventOutlookSyncStatus.textContent = "Connect Outlook Calendar first.";
  } else if (!outlookWriteReadyFlag) {
    eventOutlookSyncStatus.textContent = "Reconnect in settings to enable Outlook event sync.";
  } else if (outlookEventSyncInput.checked) {
    eventOutlookSyncStatus.textContent = "Adds this event to your Outlook Calendar.";
  } else {
    eventOutlookSyncStatus.textContent = "Keeps this event in Free Time only.";
  }
}
...
function openEventModal(mode = "create", options = {}) {
  ...
  if (mode === "edit" && activeEvent()) {
    ...
    const start = new Date(event.start);
    const end = new Date(event.end);
    const allDayRange = isWholeDayRange(start, end);
    ...
    if (outlookEventSyncInput) outlookEventSyncInput.checked = event.syncToOutlook !== false;
    ...
    if (outlookEventSyncInput) outlookEventSyncInput.checked = outlookEventSyncEnabled();
...
async function saveEvent(event) {
  ...
  const payload = {
    ...
    syncToGoogle: Boolean(eventGoogleSyncInput?.checked),
    syncToOutlook: Boolean(outlookEventSyncInput?.checked),
...
async function bootHome() {
  ...
  if (sessionInfo?.connected) {
    setStatus(homeStatus, `Calendar connected as ${sessionInfo.user?.displayName || sessionInfo.user?.name || "you"}. Join a room code or host a new room.`, "connected");
  } else if (appConfig?.googleReady || appConfig?.outlookReady) {
...
choiceConnectOutlookButton?.addEventListener("click", () => {
  if (!pendingEntryRoomCode) return;
  window.location.href = `/auth/microsoft?room=${pendingEntryRoomCode}`;
});
...
connectOutlookButton?.addEventListener("click", () => {
  if (!currentRoom?.code) return;
  window.location.href = `/auth/microsoft?room=${currentRoom.code}`;
});
settingsReconnectOutlookButton?.addEventListener("click", () => {
  if (!currentRoom?.code) return;
  window.location.href = `/auth/microsoft?room=${currentRoom.code}`;
});
...
outlookEventSyncToggle?.addEventListener("change", async () => {
  const nextEnabled = Boolean(outlookEventSyncToggle.checked);
  if (nextEnabled && !outlookWriteReady()) {
    outlookEventSyncToggle.checked = false;
    outlookEventSyncStatus.textContent = "Reconnect Outlook once to allow Free Time to create event copies.";
    return;
  }

  outlookEventSyncToggle.disabled = true;
  outlookEventSyncStatus.textContent = nextEnabled ? "Turning Outlook event sync on..." : "Turning Outlook event sync off...";
  try {
    const data = await fetchJson("/api/me/calendar-event-sync", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ provider: "outlook", enabled: nextEnabled })
    });
    sessionInfo.user = data.user;
    sessionInfo.connected = Boolean(data.user?.connected);
    renderCalendarEventSyncControls();
  } catch (error) {
    outlookEventSyncToggle.checked = outlookEventSyncEnabled();
    outlookEventSyncStatus.textContent = error.message;
  } finally {
    renderCalendarEventSyncControls();
  }
});
...
enableDialogBackdropClose(busyModal, closeBusyModal);
enableDialogBackdropClose(createRoomModal, closeCreateRoomModal);
enableDialogBackdropClose(proposeModal, closeProposeModal);
