...
function eventCardSizeClass(duration) {
  if (duration >= 1.5) return "event-large";
  if (duration > 0.75) return "event-medium";
  if (duration > 0.33) return "event-small";
  return "event-tiny";
}
...
function createEventBlock(item, dayIndex) {
  ...
  const sizeClass = eventCardSizeClass(duration);
  ...
  const tooltip = `${item.participantName} · ${item.title} · ${timeRange} · ${item.summary}`;
  block.dataset.tooltip = tooltip;
  block.title = tooltip;
