...
function wireInlineNameEditor(target, getCurrentName, onSave) {
  if (!target) return;
  target.addEventListener("dblclick", () => {
    ...
    const restore = async (shouldSave) => {
      ...
      replacement.className = "editable-identity-name";
      replacement.textContent = shouldSave ? nextName : currentName;
      input.replaceWith(replacement);
      wireInlineNameEditor(replacement, getCurrentName, onSave);
      ...
      if (busyItem.editable) {
        ...
        managePanel.innerHTML = `
          ...
          <div class="manage-privacy-row">
            <button type="button" class="secondary" data-privacy-toggle>${busyItem.visibility === "shared" ? "Public" : "Private"}</button>
        ...
        managePanel.querySelector("[data-privacy-toggle]").addEventListener("click", async (event) => {
          const nextVisibility = busyItem.visibility === "shared" ? "busy" : "shared";
          event.currentTarget.textContent = nextVisibility === "shared" ? "Public" : "Private";
