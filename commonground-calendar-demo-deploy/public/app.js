...
function renderRoomSwitcher() {
  ...
  for (const room of rooms) {
    ...
    if (isActive && currentIsHost) {
      const editButton = document.createElement("button");
      editButton.type = "button";
      editButton.className = "room-switch-edit";
      editButton.title = "Change room emoji";
      editButton.setAttribute("aria-label", "Change room emoji");
      editButton.textContent = "✎";
      editButton.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        openRoomIconEditor(editButton);
      });
...
function roomInviteMessage() {
  const roomLabel = `${currentRoom?.icon ? `${currentRoom.icon} ` : ""}${currentRoom?.name || "Free Time room"}`.trim();
...
function renderRoomMeta() {
  ...
  if (currentParticipantNeedsReconnect()) {
    ...
    connectWidgetText.textContent = currentParticipant?.lastSyncError
      ...
      : "Your calendar needs reconnecting so Free Time can keep your live availability and Google name in sync.";
    ...
    connectWidgetText.textContent = syncStamp
      ? `Your Google Calendar is connected. Free Time last synced ${calendarName} at ${syncStamp} and is using your Google profile as the default identity.`
      : "Your Google Calendar is connected. Free Time is pulling your live availability and using your Google profile as the default identity.";
  } else {
...
function createEventBlock(item, dayIndex) {
  ...
  const compactLine = [titleText, item.location].filter(Boolean).join(", ");
  const fifteenLine = [titleText, compactTime, item.location].filter(Boolean).join(", ");
...
function positionRoomIconEditor(anchor, editor) {
  ...
  editor.style.top = `${Math.min(rect.bottom + 10, window.innerHeight - 96)}px`;
}

function closeRoomIconEditor(commit = true) {
  const editor = roomIconEditor;
  if (!editor) return;
  roomIconEditor = null;
  const input = editor.querySelector("input");
  editor.remove();
  if (commit) {
    saveRoomIconValue(input?.value || "");
  }
}

function openRoomIconEditor(anchor) {
  if (!currentRoom || !currentIsHost) return;
  closeRoomIconEditor(false);

  const editor = document.createElement("div");
  editor.className = "room-icon-editor";
  editor.innerHTML = `
    <label>
      <span>Room emoji</span>
      <input type="text" maxlength="16" autocomplete="off" inputmode="text" aria-label="Room emoji" />
    </label>
  `;
  const input = editor.querySelector("input");
  input.value = currentRoom.icon || "";
  input.addEventListener("input", () => {
    input.value = normalizeRoomIconInput(input.value);
  });
  input.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      event.preventDefault();
      closeRoomIconEditor(true);
      return;
    }
    if (event.key === "Escape") {
      event.preventDefault();
  ...
  roomIconEditor = editor;
  positionRoomIconEditor(anchor, editor);
  input.focus();
  input.select();
}

async function saveRoomIconValue(value) {
  if (!currentRoom) return;
  const icon = normalizeRoomIconInput(value);
...
async function saveRoomIcon() {
  await saveRoomIconValue(roomIconInput?.value || "");
}
...
document.addEventListener("click", (event) => {
  if (roomIconEditor && !roomIconEditor.contains(event.target) && !event.target.closest(".room-switch-edit")) {
    closeRoomIconEditor(true);
  }
