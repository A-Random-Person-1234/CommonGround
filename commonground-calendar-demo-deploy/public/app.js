...
function eventCardMetrics(duration) {
  ...
  if (pixelHeight >= 84) {
    return { sizeClass: "event-large", pixelHeight, showSmallTime: true };
  }
  if (pixelHeight >= 56) {
    return { sizeClass: "event-medium", pixelHeight, showSmallTime: true };
  }
  if (pixelHeight >= 34) {
    return { sizeClass: "event-small", pixelHeight, showSmallTime: pixelHeight >= 42 };
  }
  return { sizeClass: "event-tiny", pixelHeight, showSmallTime: false };
}
...
function createBusyStack(segment, dayIndex) {
  ...
  summary.innerHTML = `
    <strong>${compactSummary ? visibleNames : `${visibleNames}${extra}`}</strong>
    <span class="busy-visibility">${segment.participants.length} busy</span>
    <span class="busy-time">${compactSummary ? "" : `${formatTime(segment.startHour)} - ${formatTime(segment.endHour)}`}</span>
  ...
  for (const [index, participant] of segment.participants.entries()) {
    ...
    card.innerHTML = `<strong>${ownerLabel}</strong><span class="busy-visibility">${visibilityLabel}</span><span class="busy-time">${formatTime(segment.startHour)} - ${formatTime(segment.endHour)}</span>`;
...
function createEventBlock(item, dayIndex) {
  ...
  const { sizeClass, showSmallTime } = eventCardMetrics(duration);
  ...
  block.dataset.tooltip = tooltip;
  block.title = tooltip;
  const compactLabel = item.title || item.participantName;
  if (sizeClass === "event-large") {
    block.innerHTML = `
      <strong class="event-owner">${item.participantName}</strong>
      <span class="event-title">${item.title}</span>
      <span class="event-time">${timeRange}</span>
    `;
  } else if (sizeClass === "event-medium") {
    block.innerHTML = `
      <span class="event-owner-pill">${participantInitials(item.participantName)}</span>
      <strong class="event-title">${item.title}</strong>
      <span class="event-time">${timeRange}</span>
    `;
  } else if (sizeClass === "event-small") {
    block.classList.toggle("show-time", showSmallTime);
    block.innerHTML = showSmallTime
      ? `<strong class="event-single">${compactLabel}</strong><span class="event-time">${timeRange}</span>`
      : `<strong class="event-single">${compactLabel}</strong>`;
  } else {
    block.innerHTML = `<strong class="event-single">${compactLabel}</strong>`;
  }
