...
function eventCardMetrics(duration) {
  ...
  const renderedHeight = Math.max(18, duration * rowHeight - blockGap);
...
function createSingleBusyCard(segment, dayIndex) {
  ...
  const compactLine = `${ownerLabel} · ${visibilityLabel}`;
  const tooltip = `${ownerLabel} · ${visibilityLabel} · ${timeRange}`;
  ...
  if (durationClass === "event-15") {
    ...
    appendLine("busy-line-title", visibilityLabel);
  } else {
    appendLine("busy-line-owner", ownerLabel);
    appendLine("busy-line-title", visibilityLabel);
