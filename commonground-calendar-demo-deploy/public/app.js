...
function eventCardMetrics(duration) {
  ...
  if (pixelHeight >= 56) {
    return { sizeClass: "event-large", pixelHeight };
  }
  if (pixelHeight >= 38) {
    return { sizeClass: "event-medium", pixelHeight };
  }
  if (pixelHeight >= 22) {
    return { sizeClass: "event-small", pixelHeight };
  }
  return { sizeClass: "event-tiny", pixelHeight };
}
...
function createEventBlock(item, dayIndex) {
  ...
  const { sizeClass } = eventCardMetrics(duration);
  block.className = `event-card ${sizeClass} ${item.isInvitee ? "invitee" : ""} ${item.isInvitedViewer ? "is-invited-viewer" : ""}`.trim();
  block.style.setProperty("--day-index", dayIndex);
  ...
  block.dataset.tooltip = tooltip;
  block.title = tooltip;
  const titleText = item.title || "Event";
  ...
  block.innerHTML = `
    ...
    <strong class="event-owner">${item.participantName}</strong>
  `;
  block.addEventListener("click", (event) => {
    if (shouldSuppressCalendarClick(event)) return;
