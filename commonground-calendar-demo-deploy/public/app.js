...
const connectWidget = document.querySelector("#connectWidget");
...
function renderRoomMeta() {
  ...
  if (currentParticipantNeedsReconnect()) {
    ...
    connectWidgetText.textContent = "Your calendar needs reconnecting to keep availability live.";
    connectWidget.classList.remove("connected");
  } else if (currentParticipantConnected()) {
    connectGoogleButton.textContent = "Reconnect calendar";
    connectWidgetText.textContent = "Live availability is connected and refreshing.";
    connectWidget.classList.add("connected");
  } else {
    connectGoogleButton.textContent = "Connect calendar";
    connectWidgetText.textContent = "Connect your calendar to see live availability.";
    connectWidget.classList.remove("connected");
  }
}
...
function buildSelfEditPanel() {
  ...
  const currentColorOption = participantPalette.find((option) => option.value === currentParticipant.color) || participantPalette[0];
  const wrap = document.createElement("section");
  wrap.className = "self-edit-panel";
  wrap.innerHTML = `
    <div class="self-edit-head">
      <div>
        <p class="panel-label">Your calendar block</p>
        <p class="muted">Update your name or color here.</p>
      </div>
    </div>
    <div class="name-form">
      <input type="text" id="busyEditNameInput" value="${currentParticipant.displayName}" autocomplete="off" />
    </div>
    <button class="secondary" type="button" id="addLocalBusyButton">Add local busy block</button>
    <label class="color-picker-row">
      <span class="current-color-dot" style="--swatch-color: ${currentColorOption.value}"></span>
      <select class="color-select" id="busyEditColorSelect" aria-label="Choose your color">
        ${participantPalette.map((option) => `
          <option value="${option.value}" ${option.value === currentParticipant.color ? "selected" : ""}>
            ${option.name}
          </option>
        `).join("")}
      </select>
  ...
  const input = wrap.querySelector("#busyEditNameInput");
  let saveTimer = null;
  input.addEventListener("input", () => {
    window.clearTimeout(saveTimer);
    saveTimer = window.setTimeout(() => {
      saveDisplayName(input.value.trim());
    }, 220);
  });

  wrap.querySelector("#addLocalBusyButton").addEventListener("click", async () => {
    const start = window.prompt("Start time (YYYY-MM-DDTHH:MM)", "");
    const end = window.prompt("End time (YYYY-MM-DDTHH:MM)", "");
    if (!start || !end) return;
  ...
  const colorSelect = wrap.querySelector("#busyEditColorSelect");
  const currentColorDot = wrap.querySelector(".current-color-dot");
  colorSelect.addEventListener("change", async () => {
    currentColorDot.style.setProperty("--swatch-color", colorSelect.value);
    await saveParticipantColor(colorSelect.value);
  });
  ...
  for (const entry of group.participants) {
    ...
    for (const busyItem of (entry.items || []).sort((a, b) => new Date(a.start) - new Date(b.start))) {
      ...
      if (busyItem.editable) {
        ...
        visibilityButton.textContent = busyItem.visibility === "shared" ? "Visible to room" : "Keep private";
        visibilityButton.addEventListener("click", async () => {
          await updateAvailabilityItem(busyItem.sourceId, {
            visibility: busyItem.visibility === "shared" ? "busy" : "shared"
          });
        });
        ...
        editButton.addEventListener("click", async () => {
          const nextTitle = window.prompt("Title", busyItem.title || "Busy");
          if (nextTitle === null) return;
          const nextLocation = window.prompt("Location", busyItem.location || "");
          if (nextLocation === null) return;
          const nextDescription = window.prompt("Description", busyItem.description || "");
...
async function proposeNewTime() {
  const event = activeEvent();
  if (!currentRoom || !selectedEventId || !event) return;

  const currentStart = new Date(event.start);
  const currentEnd = new Date(event.end);
  const nextDate = window.prompt("New date (YYYY-MM-DD)", event.date);
  if (!nextDate) return;
  ...
  try {
    const data = await fetchJson(`/api/rooms/${currentRoom.code}/events/${selectedEventId}/respond`, {
      ...
      body: JSON.stringify({
        response: "maybe",
        proposedStart: new Date(`${nextDate}T${nextStart}`).toISOString(),
        proposedEnd: new Date(`${nextDate}T${nextEnd}`).toISOString()
      })
    });
...
async function bootHome() {
  showHome();
  await loadConfigAndSession();
  const defaultGoogleName = sessionInfo?.user?.name || sessionInfo?.displayName || "";
  if (defaultGoogleName) {
    if (!createDisplayName.value.trim()) createDisplayName.value = defaultGoogleName;
    if (!joinDisplayName.value.trim()) joinDisplayName.value = defaultGoogleName;
  }
