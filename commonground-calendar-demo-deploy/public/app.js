...
function createBusyStack(segment, dayIndex) {
  ...
  summary.innerHTML = `
    ...
    <span class="busy-visibility">${compactSummary ? visibleNames : `${visibleNames}${extra}`}</span>
  ...
  for (const [index, participant] of segment.participants.entries()) {
    ...
    card.innerHTML = `<strong>${ownerLabel}</strong><span class="busy-visibility">${visibilityLabel}</span>`;
