...
function createSingleBusyCard(segment, dayIndex) {
  ...
  block.innerHTML = `<strong>${participant.ownerName}</strong><span>${formatTime(segment.startHour)} - ${formatTime(segment.endHour)} busy</span>`;
...
function createBusyStack(segment, dayIndex) {
  ...
  summary.innerHTML = `
    ...
    <span>${formatTime(segment.startHour)} - ${formatTime(segment.endHour)}</span>
    <div class="names">${visibleNames}${extra}</div>
...
function createEventBlock(item, dayIndex) {
  ...
  block.style.setProperty("--duration", item.endHour - item.startHour);
  block.innerHTML = `<strong>${item.title}</strong><span>${formatTime(item.startHour)} - ${formatTime(item.endHour)} · ${item.summary}</span>`;
