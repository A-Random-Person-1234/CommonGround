...
function renderInviteePicker(selectedIds = defaultInviteeIds()) {
  ...
  inviteePicker.innerHTML = `
    <div class="invitee-picker-head">
      ...
      <p class="muted">${invitees.length ? "Pick who should get this invite." : "No participants are in this room yet."}</p>
    </div>
  `;
...
function eventCardMetrics(duration) {
  ...
  const pixelHeight = Math.max(duration * rowHeight - 10, 18);
...
function createEventBlock(item, dayIndex) {
  ...
  const tooltip = [item.participantName, item.title, timeRange, item.summary].filter(Boolean).join("\n");
  block.dataset.tooltip = tooltip;
  block.title = tooltip;
...
function openEventModal(mode = "create", options = {}) {
  ...
  if (mode === "edit" && activeEvent()) {
    ...
    setAllDayMode(isWholeDayRange(start, end));
...
async function saveEvent(event) {
  ...
  const end = allDay ? new Date(`${date}T00:00`) : new Date(`${date}T${eventEndInput.value}`);
