...
const roomIcon = document.querySelector("#roomIcon");
...
const quickRoomIconInput = document.querySelector("#quickRoomIconInput");
...
const roomIconInput = document.querySelector("#roomIconInput");
...
function renderRoomSwitcher() {
  ...
  for (const room of rooms) {
    ...
    emoji.className = "room-switch-emoji";
    emoji.textContent = room.emoji || room.icon || "📅";
    ...
    button.append(emoji, label, meta);
    button.addEventListener("click", async () => {
      await switchRoom(code);
    });
    roomSwitcher.appendChild(button);
  }
  ...
  addButton.innerHTML = `
    <span class="room-switch-emoji" aria-hidden="true">+</span>
...
function roomInviteMessage() {
  const roomLabel = `${currentRoom?.icon ? `${currentRoom.icon} ` : ""}${currentRoom?.name || "CommonGround room"}`.trim();
  ...
  return [
    `Join me on CommonGround for ${roomLabel}.`,
...
function normalizeRoomIconInput(value) {
...
function busyVisibilityLabel(participant, isOwnBlock) {
  ...
  return ownBusyLabel(participant) || "Busy";
}
  ...
  for (const request of requests) {
    ...
    row.innerHTML = `
      <div class="request-copy">
        ...
        <span>${request.source === "google" ? "Google" : "Guest"}${requestedAt ? ` · ${requestedAt}` : ""}</span>
      </div>
...
function renderRoomMeta() {
  ...
  const nextIcon = currentRoom?.icon || "";
  roomIcon.textContent = nextIcon;
  roomIcon.classList.toggle("hidden", !nextIcon);
  ...
  syncInputValue(roomIconInput, currentRoom?.icon || "");
  ...
  if (currentParticipantNeedsReconnect()) {
    ...
    connectWidgetText.textContent = currentParticipant?.lastSyncError
      ...
      : "Your calendar needs reconnecting so CommonGround can keep your live availability and Google name in sync.";
    ...
    connectWidgetText.textContent = syncStamp
      ? `Your Google Calendar is connected. CommonGround last synced ${calendarName} at ${syncStamp} and is using your Google profile as the default identity.`
      : "Your Google Calendar is connected. CommonGround is pulling your live availability and using your Google profile as the default identity.";
  } else {
...
function eventCardMetrics(duration) {
  ...
  const pixelHeight = Math.max(duration * rowHeight, 12);
  const minutes = Math.round(duration * 60);
  let durationClass = "event-60plus";

  if (minutes <= 20 || pixelHeight < 22) {
    durationClass = "event-15";
  } else if (minutes <= 35 || pixelHeight < 34) {
    durationClass = "event-30";
  } else if (minutes <= 50 || pixelHeight < 48) {
    durationClass = "event-45";
  }

  if (pixelHeight >= 88) {
    return { sizeClass: "event-large", durationClass, pixelHeight };
  }
  if (pixelHeight >= 52) {
    return { sizeClass: "event-medium", durationClass, pixelHeight };
  }
  if (pixelHeight >= 30) {
    return { sizeClass: "event-small", durationClass, pixelHeight };
  }
  return { sizeClass: "event-tiny", durationClass, pixelHeight };
}
...
function createSingleBusyCard(segment, dayIndex) {
  ...
  block.className = `busy-card ${isCompact ? "compact" : ""} ${isTiny ? "tiny" : ""} ${hasInvitedOverlap ? "invited-overlap" : ""}`.trim();
  ...
  const tooltip = `${ownerLabel} · ${visibilityLabel} · ${formatTime(segment.startHour)} - ${formatTime(segment.endHour)}`;
  block.dataset.tooltip = tooltip;
  block.title = tooltip;
  block.innerHTML = isTiny
    ? `<strong>${ownerLabel.slice(0, 1)}</strong>`
    : isCompact
      ? `<strong>${ownerLabel}</strong>`
      : `<strong>${ownerLabel}</strong><span class="busy-visibility">${visibilityLabel}</span><span class="busy-time">${formatTime(segment.startHour)} - ${formatTime(segment.endHour)}</span>`;
...
function createBusyStack(segment, dayIndex) {
  const duration = segment.endHour - segment.startHour;
  const { sizeClass } = eventCardMetrics(duration);
  const hasInvitedOverlap = segment.participants.some((participant) => invitedEventOverlap(participant.participantId, segment.date, segment.startHour, segment.endHour));
  const stack = document.createElement("div");
  stack.className = `busy-stack ${hasInvitedOverlap ? "invited-overlap" : ""}`.trim();
...
function createEventBlock(item, dayIndex) {
  ...
  const laneWidthPercent = columnWidthPercent / laneCount;
  const laneLeftPercent = columnWidthPercent * dayIndex + laneWidthPercent * laneIndex;
  ...
  const timeRange = `${formatTime(item.startHour)} - ${formatTime(item.endHour)}`;
  const compactTime = formatTime(item.startHour);
  const tooltip = [item.title, timeRange, item.location, item.participantName, item.summary].filter(Boolean).join("\n");
  block.dataset.tooltip = tooltip;
  block.title = tooltip;
  const titleText = item.title === "No title" ? "(No title)" : (item.title || "(No title)");
  const compactLine = [titleText, compactTime, item.location].filter(Boolean).join(", ");
  const shortDetail = [compactTime, item.location].filter(Boolean).join(", ");
  block.innerHTML = `
    <span class="event-compact">${compactLine}</span>
    <span class="event-title">${titleText}</span>
    <span class="event-short-detail">${shortDetail}</span>
...
function createFreeGlowBlock(segment, dayIndex) {
  ...
  const labelHeightHours = segment.endHour - segment.startHour >= 2 ? 1.35 : 0.95;
  ...
  const block = document.createElement("button");
  block.type = "button";
  block.className = `free-glow-block${labelOverlap ? " free-glow-block--label-hidden" : ""}`;
  block.style.setProperty("--day-index", dayIndex);
  block.style.setProperty("--start", segment.startHour - calendarStartHour);
  ...
  if (segment.endHour - segment.startHour >= 1 && !labelOverlap) {
...
function renderPlanner(days) {
  ...
  days.forEach((day, dayIndex) => {
    const dayBusySegments = busySegmentsForDate(day.date);
    const rawEventBlocks = eventBlocksForDate(day.date);
    const dayEventBlocks = layoutEventLanes(rawEventBlocks);
    const occupiedSegments = occupiedSegmentsForDate(day.date, dayBusySegments, rawEventBlocks);
...
function openCreateRoomModal() {
  ...
  if (quickRoomIconInput) quickRoomIconInput.value = "";
...
async function createRoomFromSwitcher(event) {
  ...
  try {
    ...
    const iconValue = normalizeRoomIconInput(quickRoomIconInput?.value || "") || "📅";
    const data = await fetchJson("/api/rooms", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
async function saveRoomIcon() {
  if (!currentRoom) return;
  const icon = normalizeRoomIconInput(roomIconInput?.value || "");
  if (icon === (currentRoom.icon || "")) return;
...
async function bootHome() {
  ...
  if (sessionInfo?.connected) {
    setStatus(homeStatus, `Google Calendar connected as ${sessionInfo.user?.displayName || sessionInfo.user?.name || "you"}. Join a room code or host a new room.`, "connected");
...
roomIconInput?.addEventListener("input", () => {
  roomIconInput.value = normalizeRoomIconInput(roomIconInput.value);
});
roomIconInput?.addEventListener("keydown", async (event) => {
  if (event.key !== "Enter") return;
  event.preventDefault();
  await saveRoomIcon();
});
roomIconInput?.addEventListener("blur", async () => {
  await saveRoomIcon();
});
...
quickRoomIconInput?.addEventListener("input", () => {
  quickRoomIconInput.value = normalizeRoomIconInput(quickRoomIconInput.value);
});
