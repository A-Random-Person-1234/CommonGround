...
function updateDetailGoogleSyncControl(canManage) {
  ...
  if (!canManage) {
    ...
    detailGoogleSyncStatus.textContent = "Reconnect in settings to enable Google event sync.";
  } else if (detailGoogleSyncInput.checked) {
    detailGoogleSyncStatus.textContent = "Adds this event to invited calendars.";
  } else {
    detailGoogleSyncStatus.textContent = "Keeps this event in Free Time only.";
  }
}
...
function renderInviteePicker(selectedIds = defaultInviteeIds(), options = {}) {
  ...
  for (const participant of invitees) {
    ...
    label.innerHTML = `
      <input type="checkbox" value="${participant.id}" ${selected.has(participant.id) || isLocked ? "checked" : ""} ${isLocked ? "disabled" : ""} />
      <span class="invitee-color-dot"></span>
      <strong>${participant.displayName}${suffix}</strong>
    `;
...
function renderCalendarEventSyncControls() {
  ...
  if (!connected) {
    ...
    googleEventSyncStatus.textContent = "Reconnect to enable event sync.";
  } else if (enabled) {
...
function renderDetailInviteeList(event, canManage) {
  ...
  for (const participant of participants) {
    ...
    row.innerHTML = `
      <input type="checkbox" value="${participant.id}" ${invited ? "checked" : ""} ${!canManage || isCreator ? "disabled" : ""} />
      <span class="invitee-color-dot"></span>
      <span class="detail-invitee-copy">
        <strong>${participant.displayName}${participant.id === currentParticipant?.id ? " (You)" : ""}</strong>
        <small>${inviteStatusLabel}</small>
      </span>
      <span class="detail-invitee-status">${invited ? responseLabel : "Not invited"}</span>
    `;
...
function setEventPanelReadOnly(readOnly) {
  for (const input of [
    detailTitleInput,
    detailDateInput,
    detailStartInput,
...
function renderEventPanelForm(event, canManage) {
  ...
  if (detailDateInput) detailDateInput.value = event.date || dateKey(start);
  ...
  setEventPanelReadOnly(!canManage);
...
function showNotification(notification) {
  ...
  while (notificationStack.children.length > 4) {
    ...
    if (!last) break;
    removeNotificationCard(last.dataset.notificationId);
  }
}
...
async function saveEventPanelChanges(event) {
  ...
  const start = new Date(`${date}T${detailStartInput.value}`);
  const end = new Date(`${date}T${detailEndInput.value}`);
  if (!Number.isNaN(start.getTime()) && !Number.isNaN(end.getTime()) && end <= start && detailEndInput.value === "00:00") {
    end.setDate(end.getDate() + 1);
  }
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || end <= start) {
    calendarStatus.textContent = "Pick a valid event time.";
