...
const hostPanelButton = document.querySelector("#hostPanelButton");
...
function renderRoomMeta() {
  ...
  hostPanelButton.classList.toggle("hidden", !currentIsHost);
  if (!currentIsHost) {
...
async function renameRoom() {
  ...
  if (!name || name === currentRoom.name) return;
...
function maybeRestoreTheme() {
  ...
  themeButton.classList.toggle("is-dark", storedTheme === "dark");
}
...
hostPanelButton.addEventListener("click", () => {
...
document.addEventListener("click", (event) => {
  ...
  if (hostPopover.contains(event.target) || hostPanelButton.contains(event.target)) return;
  hostPopover.classList.add("hidden");
});
