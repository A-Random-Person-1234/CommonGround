...
function renderInviteePicker(selectedIds = defaultInviteeIds()) {
  ...
  const invitees = (currentRoom?.participants || []).filter((participant) => participant.connected);
...
function createBusyStack(segment, dayIndex) {
  ...
  for (const [index, participant] of segment.participants.entries()) {
    ...
    card.type = "button";
    card.className = "busy-stack-item";
...
function createEventBlock(item, dayIndex) {
  ...
  const { sizeClass } = eventCardMetrics(duration);
  block.className = `event-card ${sizeClass} ${item.isInvitee ? "invitee" : ""}`.trim();
  block.style.setProperty("--day-index", dayIndex);
...
function dragSelection() {
  ...
  return normalizedDragRange(dragCreateState.originHour, dragCreateState.currentHour, 0.5);
}
...
function handleDragCreateCancel() {
  stopDragCreate();
}
...
function startDragCreate(event, day, dayIndex) {
  if (!plannerSupportsDragCreate()) return;
  if (event.button !== undefined && event.button !== 0) return;
  if (event.target.closest(".event-card, .busy-card, .busy-stack, .free-glow-block")) return;
  if (dragCreateState?.active) return;
  ...
  dragCreateState = {
    ...
    dayIndex,
    originHour,
    currentHour: clampVisibleHour(originHour + (isTouchPointer(event) ? 0.5 : 0.5))
  };
  ensureDragPreview();
...
function renderPlanner(days) {
  ...
  for (const hour of hours) {
    ...
    for (const day of days) {
      ...
      cell.addEventListener("pointerdown", (event) => startDragCreate(event, day, days.indexOf(day)));
