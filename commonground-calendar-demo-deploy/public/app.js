...
function fullyFreeParticipantsForDate(date, blocksByDate = buildBusyDayBlocks()) {
  ...
  return (currentRoom?.participants || []).filter((participant) => participant.connected && !busyParticipants.has(participant.id));
}
...
function buildSelfEditPanel() {
  ...
  wrap.innerHTML = `
    ...
    <div class="color-palette detail-palette"></div>
  `;
  ...
  for (const color of participantPalette) {
    ...
    swatch.type = "button";
    swatch.className = `color-swatch ${currentParticipant.color === color ? "active" : ""}`.trim();
    swatch.style.setProperty("--swatch-color", color);
    swatch.style.setProperty("--chip-color", currentParticipant.color);
