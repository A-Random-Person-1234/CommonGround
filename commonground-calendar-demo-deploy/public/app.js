...
function renderCalendarEventSyncControls() {
  ...
  if (!connected) {
    googleEventSyncStatus.textContent = "Connect Google Calendar to add event invites back to your calendar.";
  } else if (!writeReady) {
...
function updateEventGoogleSyncControl() {
  ...
  const writeReady = calendarWriteReady();
  const connected = currentUserConnected() && currentParticipantConnected();
  eventGoogleSyncInput.disabled = !connected || !writeReady;
