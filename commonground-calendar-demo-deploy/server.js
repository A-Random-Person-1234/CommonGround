...
function normalizeCalendarEventSync(sync = {}, scopes = []) {
  return {
    enabled: typeof sync.enabled === "boolean" ? sync.enabled : hasGoogleCalendarWriteScope(scopes),
...
function normalizeOutlookEventSync(sync = {}, scopes = []) {
  return {
    enabled: typeof sync.enabled === "boolean" ? sync.enabled : hasMicrosoftCalendarWriteScope(scopes),
    updatedAt: sync.updatedAt || nowIso()
  };
}
...
function normalizeEventRecord(event = {}) {
  return {
    ...
    syncToOutlook: event.syncToOutlook !== false,
...
function setUserGoogleConnection(userId, profile, tokens) {
  ...
  const scopes = uniqueStrings(typeof tokens?.scope === "string" ? tokens.scope.split(" ") : []);
  ...
  return updateUserRecord(userId, {
    ...
    calendarEventSync: {
      enabled: existingEventSync.enabled === false ? false : hasGoogleCalendarWriteScope(scopes),
      updatedAt: nowIso()
    },
...
function setUserMicrosoftConnection(userId, profile, tokens) {
  ...
  return updateUserRecord(userId, {
    ...
    outlookEventSync: {
      enabled: existingEventSync.enabled === false ? false : hasMicrosoftCalendarWriteScope(scopes),
      updatedAt: nowIso()
    },
    createdAt: store.users[userId]?.createdAt || nowIso()
  });
...
function roomNameToCode(roomName) {
...
function generateRoomCode(preferredName = "") {
  const preferredCode = roomNameToCode(preferredName);
  if (isValidRoomCode(preferredCode) && !store.rooms[preferredCode]) {
    return preferredCode;
  }

  if (isValidRoomCode(preferredCode)) {
    for (let attempt = 0; attempt < 80; attempt += 1) {
      const suffixLength = attempt < roomCodeAlphabet.length ? 1 : 2;
      let suffix = "";
      for (let index = 0; index < suffixLength; index += 1) {
        suffix += roomCodeAlphabet[Math.floor(Math.random() * roomCodeAlphabet.length)];
      }
      const candidate = `${preferredCode.slice(0, 16 - suffix.length)}${suffix}`;
      if (isValidRoomCode(candidate) && !store.rooms[candidate]) {
        return candidate;
      }
    }
  }
  ...
  if (store.rooms[code]) return generateRoomCode(preferredName);
  return code;
}
...
function mergeParticipantIntoRoom(room, target, source) {
  ...
  target.availabilityOverrides = {
    ...(source.availabilityOverrides || {}),
    ...(target.availabilityOverrides || {})
  };
  const existingLocalIds = new Set((target.localAvailabilityItems || []).map((item) => item.id));
  target.localAvailabilityItems = [
    ...(target.localAvailabilityItems || []),
    ...(source.localAvailabilityItems || []).filter((item) => !existingLocalIds.has(item.id))
  ];
...
function upsertNotification(notification) {
  ...
  if (existing) {
    if (!existing.read && !existing.dismissed) {
...
async function fetchPrimaryCalendarEvents(accessToken, calendarId, timeMin, timeMax) {
  ...
  url.searchParams.set("fields", "items(id,summary,location,description,start,end,transparency,status,extendedProperties)");
...
async function fetchOutlookCalendarEvents(accessToken, timeMin, timeMax) {
  ...
  url.searchParams.set("$select", "id,subject,location,start,end,bodyPreview,showAs,isCancelled");
...
function isSyncedGoogleMirrorEvent(room, detailEvent, mirroredIds) {
  ...
  if (privateProps.freeTimeRoomCode !== room.code || !privateProps.freeTimeEventId) {
    return false;
  }

  return (room.events || []).some((roomEvent) => roomEvent.id === privateProps.freeTimeEventId);
}
...
function googleCalendarEventDescription(room, event) {
  ...
  return [
    ...
    `Free Time room: ${room.name} (${room.code})`,
...
function googleCalendarPayload(room, event, ownerParticipant, includeAttendees = false) {
  ...
  const ownerUser = ownerParticipant?.userId ? store.users[ownerParticipant.userId] : null;
  ...
  return {
    summary: String(event.title || "Free Time event").trim() || "Free Time event",
    ...
    extendedProperties: {
      private: {
        freeTimeRoomCode: room.code,
        freeTimeEventId: event.id,
        freeTimeParticipantId: ownerParticipant?.id || "",
        freeTimeCreator: creator?.displayName || "",
        freeTimeOwnerEmail: ownerUser?.email || ""
      }
    },
...
function outlookCalendarPayload(room, event, ownerParticipant, includeAttendees = false) {
  ...
  return {
    subject: String(event.title || "Free Time event").trim() || "Free Time event",
    body: {
      contentType: "text",
...
async function deleteGoogleCalendarEntry(userId, entry, sendUpdates = "none") {
  if (!entry?.googleEventId || !userConnected(store.users[userId])) return;
  try {
...
async function upsertGoogleCalendarEvent(room, event, participant) {
  ...
  const includeAttendees = false;
  const sendUpdates = "none";
  const shouldSync = event.syncToGoogle !== false && user?.calendarEventSync?.enabled !== false && userHasGoogleCalendarWriteAccess(user);
...
async function deleteOutlookCalendarEntry(userId, entry, sendUpdates = null) {
  if (!entry?.outlookEventId || !userConnected(store.users[userId])) return;
  try {
...
async function upsertOutlookCalendarEvent(room, event, participant) {
  ...
  const includeAttendees = false;
  const sendUpdates = "none";
  const shouldSync = event.syncToOutlook !== false && user?.outlookEventSync?.enabled !== false && userHasMicrosoftCalendarWriteAccess(user);
...
function eventSyncParticipants(room, event) {
  return uniqueStrings([event.createdByParticipantId, ...(event.inviteeParticipantIds || [])])
    .map((participantId) => findParticipantById(room, participantId))
...
async function syncGoogleCalendarForEvent(room, event) {
  ...
  for (const [userId, entry] of Object.entries(event.googleCalendarSync)) {
    if (!targetUserIds.has(userId) || event.syncToGoogle === false) {
      await deleteGoogleCalendarEntry(userId, entry, "none");
...
async function syncOutlookCalendarForEvent(room, event) {
  ...
  for (const [userId, entry] of Object.entries(event.outlookCalendarSync)) {
    if (!targetUserIds.has(userId) || event.syncToOutlook === false) {
      await deleteOutlookCalendarEntry(userId, entry, "none");
...
async function fetchUserFreeBusy(room, user, participant, timeMin, timeMax, viewerParticipantId) {
  ...
  const microsoftTokens = await getFreshMicrosoftTokensForUser(user.id);

  if (!tokens?.access_token && !microsoftTokens?.access_token) {
    participant.syncStatus = "needs_reconnect";
    participant.lastSyncError = "No connected calendar token found.";
    updateUserSync(user.id, {
      status: "needs_reconnect",
      lastAttemptAt: nowIso(),
      lastError: "No connected calendar token found."
    });
  ...
  const providerNames = [];
  const providerErrorStatuses = [];
  const mirroredGoogleEventIds = syncedGoogleCalendarMirrorIds(room, user.id);
  const mirroredOutlookEventIds = syncedOutlookCalendarMirrorIds(room, user.id);
  ensureParticipantAvailabilityState(participant);

  if (tokens?.access_token) {
    try {
      const googleCalendarList = await fetchCalendarList(tokens.access_token);
      const primaryCalendar = googleCalendarList.calendars[0];
      const response = await fetch("https://www.googleapis.com/calendar/v3/freeBusy", {
        ...
        body: JSON.stringify({
          ...
          items: googleCalendarList.calendars.map((calendar) => ({ id: calendar.id }))
        })
      });

      const payload = await response.json();
      if (!response.ok) {
        const message = payload.error?.message || "Could not fetch Google calendar availability.";
        providerErrorStatuses.push({ provider: "google", status: response.status, message });
      } else {
        ...
        const mapped = detailEvents
          .filter((event) => (
            event.status !== "cancelled" &&
            event.transparency !== "transparent" &&
            !isSyncedGoogleMirrorEvent(room, event, mirroredGoogleEventIds)
          ))
          .map((event) => {
        ...
        providerResults.push(googleCalendarList);
      }
    } catch (error) {
      providerErrorStatuses.push({ provider: "google", status: 0, message: error.message || "Could not fetch Google calendar availability." });
    }
  }

  if (microsoftTokens?.access_token) {
    try {
      const outlookCalendar = await fetchMicrosoftPrimaryCalendar(microsoftTokens.access_token);
      const details = await fetchOutlookCalendarEvents(microsoftTokens.access_token, start, end);
      const detailEvents = (details || [])
        .filter((event) => {
          if (event.isCancelled) return false;
          if (mirroredOutlookEventIds.has(String(event.id))) return false;
          const status = String(event.showAs || "").toLowerCase();
          return status !== "free" && status !== "tentative";
        })
        .map((event) => {
          const sourceId = outlookItemSourceId(outlookCalendar?.id || "primary", event.id);
          const override = participant.availabilityOverrides[sourceId] || {};
          if (override.hidden) return null;

          const startDate = override.start ? new Date(override.start) : parseOutlookDateTime(event.start);
          const endDate = override.end ? new Date(override.end) : parseOutlookDateTime(event.end);
          if (!startDate || !endDate || Number.isNaN(startDate.getTime()) || Number.isNaN(endDate.getTime()) || endDate <= startDate) {
            return null;
          }
          ...
          const viewerCanSeeDetails =
            ...
            visibility === "shared" ||
            sharedWithParticipantIds.includes(viewerParticipantId);
          const title = override.title ?? event.subject ?? "";
          const location = override.location ?? event.location?.displayName ?? "";
          const description = override.description ?? event.bodyPreview ?? "";
          ...
          return {
            ...
            color: participant.color,
            busy: true,
            calendarId: outlookCalendar?.id || "outlook-primary",
      ...
      busy.push(...detailEvents);
      providerNames.push("outlook");
      providerResults.push({ calendars: [outlookCalendar], needsReconnect: false, calendarListError: null });
    } catch (error) {
      providerErrorStatuses.push({ provider: "outlook", status: 0, message: error.message || "Could not fetch Outlook calendar availability." });
    }
  }
  ...
  for (const entry of busy) {
    const key = `${entry.ownerId}|${entry.ownerName}|${entry.start}|${entry.end}|${entry.items?.[0]?.provider || "local"}|${entry.items?.[0]?.sourceId || ""}`;
    if (seenBusyKeys.has(key)) continue;
    seenBusyKeys.add(key);
    uniqueBusy.push(entry);
  }

  participant.connected = hasConnectedCalendar(user);
  participant.needsReconnect = false;
  participant.syncStatus = "connected";
  participant.lastSyncedAt = nowIso();
  participant.lastSyncError = null;
  const hasProviderError = providerErrorStatuses.length > 0;
  if (!providerNames.length || providerResults.length === 0) {
    participant.syncStatus = hasProviderError ? "error" : "connected";
    participant.needsReconnect = hasProviderError ? providerErrorStatuses.some((entry) => [401, 403].includes(entry.status)) : false;
    participant.lastSyncError = hasProviderError ? providerErrorStatuses[0]?.message : null;
  }
  if (providerNames.length) {
    const lastResult = providerResults[providerResults.length - 1] || {};
    updateUserSync(user.id, {
      status: participant.needsReconnect ? "needs_reconnect" : "connected",
      lastAttemptAt: nowIso(),
      lastSuccessAt: participant.needsReconnect ? null : nowIso(),
      lastError: participant.lastSyncError,
      calendarId: lastResult.calendars?.[0]?.id || null,
      calendarName: lastResult.calendars?.[0]?.summary || "Connected calendar"
    });
  }

  participant.lastCalendarId = providerResults[providerResults.length - 1]?.calendars?.[0]?.id || participant.lastCalendarId;
  participant.lastCalendarName = providerResults[providerResults.length - 1]?.calendars?.[0]?.summary || participant.lastCalendarName;
  participant.lastSeenAt = nowIso();
  saveStore();
  return {
    busy: uniqueBusy,
    connected: hasConnectedCalendar(user),
    needsReconnect: participant.needsReconnect || providerErrorStatuses.some((entry) => [401, 403].includes(entry.status)),
    calendarCount: providerResults.reduce((total, current) => total + (current.calendars || []).length, 0),
    calendarListError: hasProviderError ? providerErrorStatuses.map((entry) => `${entry.provider}: ${entry.message}`).join(" · ") : null
  };
}
...
function buildMicrosoftAuthUrl(session, roomCode) {
  const credentials = loadMicrosoftCredentials();
  if (!credentials) return null;
...
function publicEvent(event, room, viewerParticipantId = null) {
  ...
  const viewerIsHost = hostParticipantIds(room).includes(viewerParticipantId);
  const canSeeDetails = viewerParticipantId === event.createdByParticipantId || inviteeParticipantIds.includes(viewerParticipantId) || viewerIsHost;
  ...
  return {
    ...
    syncToGoogle: event.syncToGoogle !== false,
    syncToOutlook: event.syncToOutlook !== false,
    googleCalendarSyncStatus: Object.fromEntries(
      Object.entries(event.googleCalendarSync || {})
        .map(([userId, entry]) => [userId, {
          status: entry.status || "pending",
          lastSyncedAt: entry.lastSyncedAt || null,
          lastError: entry.lastError || null
    ...
    outlookCalendarSyncStatus: Object.fromEntries(
      Object.entries(event.outlookCalendarSync || {})
        .map(([userId, entry]) => [userId, {
          status: entry.status || "pending",
          lastSyncedAt: entry.lastSyncedAt || null,
          lastError: entry.lastError || null
...
const server = http.createServer(async (req, res) => {
  ...
  try {
      ...
      if (enabled && !writeReady) {
        sendJson(res, 409, {
          error: provider === "google"
            ? "Reconnect Google Calendar to allow Free Time to add events."
            : "Reconnect Outlook to allow Free Time to add events.",
          needsReconnect: true,
          user: publicUser(user)
        });
        return;
    ...
    if (url.pathname === "/api/rooms" && req.method === "POST") {
      ...
      const code = generateRoomCode(roomName === "Untitled room" ? "" : roomName);
      const time = nowIso();
    ...
    if (roomCodeMatch && req.method === "PATCH") {
      ...
      if (!name && !nextCode && nextLocked === null) {
        sendJson(res, 400, { error: "Room name, code, or lock state is required." });
        return;
      }
    ...
    if (roomEventsMatch && req.method === "POST") {
      ...
      const event = {
        ...
        syncToOutlook: body.syncToOutlook !== false,
        googleCalendarSync: {},
        outlookCalendarSync: {},
        createdAt: nowIso(),
        responses: {},
    ...
    if (roomEventIcsMatch && req.method === "GET") {
      ...
      const ics = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//Free Time//Shared Calendar//EN",
        "BEGIN:VEVENT",
    ...
    if (url.pathname === "/auth/microsoft") {
      const session = getSession(req, res);
      const roomCode = normalizeRoomCode(url.searchParams.get("room") || session.lastRoomCode || "");
      const authUrl = buildMicrosoftAuthUrl(session, roomCode || null);
      if (!authUrl) {
        sendRedirect(res, "/?error=missing_microsoft_credentials");
        return;
      }
      sendRedirect(res, authUrl);
      return;
      ...
      try {
        ...
        if (roomCode && store.rooms[roomCode]) {
          ...
          if (isRoomLocked(room) && !canJoinLockedRoom(room, session, store.users[profile.id])) {
            const source = userGoogleConnected(store.users[profile.id]) ? "google" : (userMicrosoftConnected(store.users[profile.id]) ? "microsoft" : "guest");
            const request = upsertJoinRequest(room, session, store.users[profile.id], session.pendingDisplayName, source);
            notifyHostsOfJoinRequest(room, request);
            room.updatedAt = nowIso();
            saveStore();
    ...
    if (url.pathname === "/auth/microsoft/callback") {
      ...
      try {
        ...
        session.userId = profile.id;
        const userRecord = setUserMicrosoftConnection(profile.id, profile, microsoftTokens);
        linkSessionParticipantsToUser(session, userRecord);
        propagateUserIdentityToRooms(profile.id);
        ...
        if (roomCode && store.rooms[roomCode]) {
          ...
          session.lastRoomCode = roomCode;
          if (isRoomLocked(room) && !canJoinLockedRoom(room, session, store.users[profile.id])) {
            const request = upsertJoinRequest(room, session, store.users[profile.id], session.pendingDisplayName, "microsoft");
            notifyHostsOfJoinRequest(room, request);
            room.updatedAt = nowIso();
            saveStore();
            sendRedirect(res, "/?request=sent");
          ...
          saveStore();
          sendRedirect(res, `/room/${roomCode}?connected=microsoft`);
          return;
        }

        saveStore();
        sendRedirect(res, "/?connected=microsoft");
      } catch (authError) {
        sendRedirect(res, `/?error=${encodeURIComponent(authError.message)}`);
      }
      return;
...
server.listen(port, host, () => {
  console.log(`Free Time running at http://localhost:${port}`);
