...
async function fetchUserFreeBusy(user, participant, timeMin, timeMax, viewerParticipantId) {
  ...
  const busy = detailEvents
    ...
    .map((event) => {
      ...
      const viewerCanSeeDetails = participant.id === viewerParticipantId || visibility === "shared";
...
async function fetchRoomFreeBusy(room, timeMin, timeMax, viewerParticipantId) {
  ...
  for (const participant of room.participants) {
    ...
    try {
      const result = await fetchUserFreeBusy(user, participant, timeMin, timeMax, viewerParticipantId);
