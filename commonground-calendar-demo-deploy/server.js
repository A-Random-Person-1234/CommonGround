...
function userHasGoogleCalendarWriteAccess(user) {
  return Boolean(userConnected(user) && hasGoogleCalendarWriteScope(user?.auth?.google?.scopes || user?.googleScopes || []));
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === "/api/me/calendar-event-sync" && req.method === "PATCH") {
      ...
      const user = currentUserFromSession(session);
      if (!user) {
        sendJson(res, 401, { error: "Connect Google Calendar before enabling event sync." });
        return;
      }

      const body = await readJsonBody(req);
      const enabled = Boolean(body.enabled);
      if (enabled && !userHasGoogleCalendarWriteAccess(user)) {
        sendJson(res, 409, {
          error: "Reconnect Google Calendar to allow Free Time to add events.",
          needsReconnect: true,
          user: publicUser(user)
        });
        return;
      ...
      const nextUser = updateUserRecord(user.id, {
        calendarEventSync: {
          enabled,
          updatedAt: nowIso()
        }
    ...
    if (roomEventsMatch && req.method === "POST") {
      ...
      auth.room.updatedAt = nowIso();
      await syncGoogleCalendarForEvent(auth.room, event);
      saveStore();
