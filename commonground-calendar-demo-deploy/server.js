...
function normalizeRoomRecord(room = {}) {
  ...
  return {
    ...
    structureVersion: Math.max(Number(room.structureVersion || 0), 2),
...
function userDisplayName(user) {
  return String(user?.profile?.preferredDisplayName || user?.displayName || userGoogleName(user)).trim() || userGoogleName(user);
}
...
function preferredDisplayNameSource(user) {
  return userDisplayName(user) === userGoogleName(user) ? "google" : "custom";
}
...
function setUserGoogleConnection(userId, profile, tokens) {
  return updateUserRecord(userId, {
    ...
    profile: {
      ...
      picture: profile.picture,
      preferredDisplayName: store.users[userId]?.profile?.preferredDisplayName || profile.name || profile.email || "Guest",
