...
function eventTimePickerOptions(picker) {
  const startMinutes = timeInputValueToMinutes(eventStartInput?.value);
...
function syncEventTimePickerDisplays() {
  eventTimePickers.forEach(syncEventTimePickerDisplay);
}
...
function commitEventTimePickerInput(picker, { showFeedback = true } = {}) {
  ...
  if (minutes === null) {
    ...
    if (showFeedback) setEventFormFeedback(message, "error");
    return false;
  }

  setEventTimePickerCanonicalValue(picker, minutes);
  if (showFeedback && eventFormFeedback?.classList.contains("is-error")) {
    setEventFormFeedback();
  }
  if (picker.kind === "start" && activeEventTimePicker?.kind === "end") {
...
function openEventTimePicker(picker) {
  if (picker.displayInput.disabled || eventAllDayInput?.checked) return;
...
function selectEventTimePickerOption(picker, option) {
  ...
  if (eventFormFeedback?.classList.contains("is-error")) setEventFormFeedback();
...
function initializeEventTimePickers() {
  eventTimePickers.forEach((picker) => {
    ...
    picker.displayInput.addEventListener("input", () => {
      ...
      if (parsedMinutes !== null && activeEventTimePicker === picker) {
        const targetMinutes = picker.kind === "end" && parsedMinutes === 0 && timeInputValueToMinutes(eventStartInput.value) !== null
          ? 1440
          : parsedMinutes;
