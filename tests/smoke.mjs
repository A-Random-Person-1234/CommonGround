...
try {
  ...
  assert.match(home.text, /href="\/styles\.css\?v=20260806-overlap-lanes"/);
  ...
  assert.match(home.text, /src="\/app\.js\?v=20260806-overlap-lanes"/);
  ...
  assertInOrder(
    ...
    [
      ...
      'href="/styles.css?v=20260806-overlap-lanes"',
      'src="/date-picker.js?v=20260726-shared-date-picker"',
      'src="/app.js?v=20260806-overlap-lanes"'
    ],
  ...
  assert.match(
    eventComposerScript.text,
    /const refreshed = await refreshRoomAfterGoogleConnection\(\);[\s\S]*?!calendarWriteReady\(\)[\s\S]*?eventGoogleSyncInput\.checked = true/,
    "A successful popup must complete a post-OAuth calendar refresh before enabling event sync"
  );
  assert.match(
    eventComposerScript.text,
    /async function refreshRoomAfterGoogleConnection\(\{ maxAttempts = 3 \} = \{\}\)[\s\S]*?if \(googleConnectionRefreshPromise\) return googleConnectionRefreshPromise;[\s\S]*?await refreshRoomData\(\)[\s\S]*?googleAvailabilityRefreshComplete\(\)[\s\S]*?waitForGoogleConnectionRetry\(250 \* \(attempt \+ 1\)\)/,
    "Post-OAuth availability refreshes must be single-flight and retry while a rotated guest session settles"
  );
  assert.match(
    eventComposerScript.text,
    /function googleAvailabilityRefreshComplete\(\)[\s\S]*?isGoogleConnected\(\)[\s\S]*?currentParticipantConnected\(\)[\s\S]*?!currentParticipantNeedsReconnect\(\)[\s\S]*?currentParticipant\?\.lastSyncedAt/,
    "Post-OAuth refresh success must require a completed provider availability sync"
  );
  assert.match(
    eventComposerScript.text,
    /async function bootRoom\(\)[\s\S]*?googleConnectionReturnPending\(\)[\s\S]*?await refreshRoomAfterGoogleConnection\(\)[\s\S]*?clearGoogleConnectionReturnParams\(\)[\s\S]*?startAutoRefresh\(\)/,
    "Full-page Google OAuth returns must sync availability immediately before normal polling resumes"
  );
  assert.match(
    eventComposerScript.text,
  ...
  assert.match(
    eventComposerScript.text,
    /function eventBlocksForDate\(date\)[\s\S]*?participantName: event\.createdByDisplayName \|\| "Someone"[\s\S]*?function createEventBlock/,
    "Manual event blocks must carry their creator's room display name into the shared day/week renderer"
  );
  assert.match(
    eventComposerScript.text,
  ...
  assertLaneGeometry(3, 1, 0.04, 0.4575, "First three-event foreground card");
  assertLaneGeometry(3, 2, 0.5225, 0.4575, "Second three-event foreground card");
  ...
  assert.match(
    eventComposerScript.text,
    /function calendarLaneGeometry\(laneCount = 1, laneIndex = 0\)[\s\S]*?leftFraction: 0,[\s\S]*?widthFraction: 1[\s\S]*?foregroundLaneCount = safeLaneCount - 1[\s\S]*?innerGap = 0\.025[\s\S]*?foregroundLaneCount === 1[\s\S]*?availableWidth \/ foregroundLaneCount[\s\S]*?foregroundLaneIndex \* \(widthFraction \+ innerGap\)/,
    "Simultaneous foreground clashes must use separate side-by-side lanes while a single overlay remains broad"
  );
  assert.match(
    eventComposerScript.text,
  ...
  for (const legalPage of [privacyPage, termsPage]) {
    ...
    assert.match(legalPage.text, /<link rel="stylesheet" href="\/styles\.css\?v=20260806-overlap-lanes" \/>/);
    assert.doesNotMatch(
      legalPage.text,
      /<script(?![^>]*\bsrc=)[^>]*>/i,
  ...
  assert.equal(createdEvent.payload.event.createdByDisplayName, "Host");
  ...
  assert.equal(spectatorEvent.createdByDisplayName, "Host");
